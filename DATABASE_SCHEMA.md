...
### `goal_milestones`
`goal_id` (fk), `title`, `target_date`, `completed_at` (nullable).

### `tasks`
`title`, `description`, `pillar_id` (nullable), `project_id` (nullable, fk
to `projects`), `priority`, `due_date`, `estimated_effort`, `status`
(`inbox | today | this_week | someday | waiting | completed`), `source`
(`manual | ai_recommended | calendar | email | goal | weekly_review |
morning_briefing`), `recurrence`, `notes`.

### `habits`
`name`, `pillar_id`, `frequency`, `target`, `reminder_time`.

### `habit_entries`
`habit_id` (fk), `entry_date`, `completed` (bool). Streaks and success rate
are **computed**, not stored — derived from `habit_entries` at read time (or
via a materialized view later) so they can't drift out of sync.
...
`tomorrow_focus`, `day_rating` (1–10).
...
e.g. `0001_init_profiles_and_pillars.sql`,
`0002_goals_tasks_habits.sql`, etc. — never hand-edit the schema outside a
migration, per the brief's "no undocumented manual database changes" rule.
Phase 0 does not create any migration files yet since there's no Supabase
project to run them against; Phase 1 starts this file sequence.
